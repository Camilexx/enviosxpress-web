-- Actualizar función calculate_shipping_price para esquema actual
-- Sin shipment_type - usa la misma tarifa para documento y carga, recargo por peso solo para carga > 5kg
...
BEGIN
    ...
    -- Obtener tarifa (sin shipment_type)
    SELECT * INTO v_rate 
    FROM shipping_rates 
    ...
    IF NOT FOUND THEN
        -- Fallback: buscar cualquier tier
        SELECT * INTO v_rate 
        FROM shipping_rates 
        WHERE zone_name = v_city.zone_name 
            AND is_active = true
    ...
    IF NOT FOUND THEN
        RETURN QUERY SELECT 
            false, 
            'Tarifa no encontrada para zona ' || v_city.zone_name, 
            NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL;
        RETURN;
    END IF;
    ...
    -- Descuento de zona (ya aplicado en precio base para zona norte)
    -- El descuento ya está incluido en base_price, así que no aplicamos doble descuento
    v_descuento_zona_pct := v_zone.discount_pct;
    v_descuento_zona_amount := 0; -- Ya aplicado en precio base
